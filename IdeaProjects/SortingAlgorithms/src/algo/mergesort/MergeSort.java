...
public class MergeSort {
    ...
    public static void merge(int inputArray[], int start, int mid, int end){
        ...
        // That above loop has completed. so both the right and the left side of the sub-array can be consider sorted

        if (leftSlot <= mid){ //consider the right dide done being sorted. left must be sorted already
            while (leftSlot <= mid){
                tempArray[k] = inputArray[leftSlot];
                leftSlot = leftSlot +1;
                k = k + 1;
